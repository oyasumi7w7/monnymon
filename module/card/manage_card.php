...
if(empty($_SESSION['valid_user']) or $_SESSION['valid_type']!=1){
 ...
 if($do==1){
   echo '<script type="text/javascript">
         swal("", "เพิ่มการ์ดเรียบร้อย", "success");
         </script>';
 }elseif($do==2){
    echo '<script type="text/javascript">
    ...
    echo '<script type="text/javascript">
    swal("", "ลบการ์ดเสร็จสิ้น", "success");
    </script>';
 }
?>
<meta charset="UTF-8">
<p align="center"><a style="font-size:50px; color:#A52A2A; ">จัดการข้อมูลการ์ด</a></p>
...
<br>
	...
	</form><a href="index.php?module=card&action=manage_card"><button class="btn btn-success btn-ra"><i class="fa fa-star"></i>&nbsp; ทั้งหมด</button></a>
<?php
	...
	$result=mysqli_query($con,"SELECT card_name FROM card_list WHERE card_name LIKE '%$search%' ")or die("SQL Error==>".mysqli_error($con));
	...
	if($rows==0){ //ถ้าคำค้นไม่ตรงกับสินค้าใดๆ
		echo "<p style='color:#00008B; font-size:20px; '>ไม่พบการ์ดที่ตรงกับคำค้น</p>";
	}
	else{
		echo "<p style='color:#00008B; font-size:20px; '>จำนวนการ์ดทั้งหมดที่ตรงกับคำค้น $rows รายการ</p>";
	
	$result=mysqli_query($con,"SELECT card_id,card_name,grade,Clan,card_type,card_pic FROM card_list WHERE card_name LIKE '%$search%'
	...
	for($i=1;$i<=$pages;$i++){ //วนลูปตามจำนวนหน้า
		...
		{
	echo"<a href='index.php?module=card&action=manage_card&pid=$i&search=$search&clan=$c_id'>[ $i ]</a>"; //สร้าง link หมายเลขหน้า
		}
	}
	echo "</p>";
	...
	<i class="fa fa-star"></i>&nbsp; เพิ่มการ์ด</button></a>
		...
		<div class="table-responsive table--no-card m-b-30">
			<table class="table table-borderless table-striped table-earning">
				<thead>
			<?php
	echo"<tr><th style='text-align:center;'width='5%'><a href='index.php?module=card&action=manage_card&tx=$_GET[tx]'>$link</a></th>
...
?>	
	...
	<tbody>
	<?php
	while(list($card_id,$card_name,$grade,$Clan,$type,$card_pic)=mysqli_fetch_row($result)){
	...
	echo "<td style='text-align:center;'>$grade</td>"; 
		$result2=mysqli_query($con,"SELECT clan_name FROM card_clan WHERE clan_id='$Clan'")or die 
            ("SQL Error2=>".mysqli_error($con));
		list($Clan)=mysqli_fetch_row($result2);
						
	echo "<td style='text-align:center;'> $Clan </td>";
