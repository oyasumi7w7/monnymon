...
<?php
	...
	for($i=1;$i<=$pages;$i++){ //วนลูปตามจำนวนหน้า
		...
		{
	echo"<a href='index.php?module=item&action=warehouse_category&pid=$i&search=$search&cate=$cate'>[ $i ]</a>"; //สร้าง link หมายเลขหน้า
		}
	}
	echo "</p>";
	
	
    $stock_pd=mysqli_query($con,"SELECT product_id,product_name FROM product WHERE product_category LIKE '%$cate%'
	ORDER BY product_name DESC LIMIT $start_rows,$rows_page  ")or die("Sql Error>>".mysqli_error($con)); 
    ?>
